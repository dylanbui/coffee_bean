...
extension NetworkMappingCommon<T> on Future<Response<T>> {
    ...
    Future<DbResult<R>> mapToData<R>() async {
        try {
            final response = await this;
            return DbSuccess(response.data as R);
    ...
    // Future<DbResult<R>> mapToObject<R>(JsonMapper<dynamic> mapper) async {
    //     try {
    //         final response = await this;
    ...
    //             return DbSuccess(list as R);
    //         }
    //         return DbFailure(NetworkError(500, "Invalid Data format (Map/List)"));
    //     } on DioException catch (ex) {
    //         return DbFailure(NetworkError(ex.response?.statusCode ?? 500, ex.message ?? "Error Connect"));
    //     } catch (e) {
    //         return DbFailure(NetworkError(500, e.toString()));
    ...
    Future<DbResult<R>> superMapToObject<R, M>(JsonMapper<M> mapper) async {
        try {
            final response = await this;
            final rawData = response.data;
            // Check data for mapper
    ...
    Future<DbResult<R>> mapToObject<R>(R Function(Map<String, dynamic>) mapper) async {
      try {
        final response = await this;
        final rawData = response.data;
        // Check data for mapper
...
/// Xử lý trả về một Danh sách Object
  Future<DbResult<List<R>>> mapToObjectList<R>(R Function(Map<String, dynamic>) mapper) async {
    try {
      final response = await this;
      final rawData = response.data;
      ...
      if (rawData is List) {
        ...
        final list = rawData.map((e) => mapper(e as Map<String, dynamic>)).toList();
        return DbSuccess(list);
      }

      return DbFailure(NetworkError(500, "Server không trả về List"));
    } on DioException catch (ex) {
      return DbFailure(NetworkError(ex.response?.statusCode ?? 500, ex.message ?? "Lỗi kết nối"));
    } catch (e) {
      return DbFailure(NetworkError(500, e.toString()));
    }
  }
...
class NetworkParsingUtils {
  ...
  static DbResult<M> parseToObject<M>(dynamic rawData, JsonMapper<M> mapper) {
    try {
      if (rawData == null) return DbFailure(NetworkError(404, "Data is null"));
      if (rawData is Map<String, dynamic>) {
        return DbSuccess(mapper(rawData));
      } else if (rawData is List) {
        if (rawData.isNotEmpty) {
          return DbSuccess(mapper(rawData.first as Map<String, dynamic>));
        }
        return DbFailure(NetworkError(500, "The list is empty, no first element found"));
      }
      return DbFailure(NetworkError(500, "Invalid data format (Not Map/List)"));
    } catch (e) {
      return DbFailure(NetworkError(500, e.toString()));
    }
  }
  ...
  static DbResult<List<M>> parseToList<M>(dynamic rawData, JsonMapper<M> mapper) {
    try {
      if (rawData == null) return DbSuccess(<M>[]);
      if (rawData is List) {
        // Using the global parseList function to handle fault tolerance
        return DbSuccess(parseList(rawData, mapper));
...
class NetworkDataMapper<T, M> {
  ...
  Future<DbResult<M>> toObject() async {
    try {
      ...
      return DbFailure(NetworkError(ex.response?.statusCode ?? 500, ex.message ?? "Connection Error"));
    } catch (e) {
      return DbFailure(NetworkError(500, e.toString()));
    }
  }
